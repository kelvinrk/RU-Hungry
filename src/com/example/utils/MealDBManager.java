...
public class MealDBManager {
	private DBhelper helper;
	private SQLiteDatabase db;
	public String strwhere = "";

	public MealDBManager(Context context) {
		helper = new DBhelper(context);
		db = helper.getWritableDatabase();
	}
	
	public void open() {
		db = helper.getWritableDatabase();/* connect to the database */
	}
	...
	public List<Meal> query() {
		ArrayList<Meal> dt_goods = new ArrayList<Meal>();
		Cursor c = queryTheCursor(strwhere);
		while (c.moveToNext()) {
			Meal goods1 = new Meal();
...
//			Log.v("Test", "Result = " + result);
	...
	public void update(Meal goods1, int val) {
		ContentValues values = new ContentValues();

		values.put("_buynumber", val);
		db.update("[dt_goods]", values, 
				"_id" + " = ?",
				new String[] { String.valueOf(goods1.get_id())});
	}
	
	public List<Meal> findAll() {
		List<Meal> dt_goods1 = new ArrayList<Meal>();
		
		Cursor c = db.query("dt_goods", 
