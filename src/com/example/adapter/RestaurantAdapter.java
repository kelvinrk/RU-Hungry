...
public class RestaurantAdapter extends BaseAdapter {
	private DiquActivity context;// Receive Context
	...
	private MyApplication myapplication1 = new MyApplication();

	public void setFlagBusy(boolean busy) {
		this.mBusy = busy;
	}

	public RestaurantAdapter(DiquActivity context, List<Restaurant> list) {
		super();
	...
	public void addlist(List<Restaurant> addlist) {
		try {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			if (!mBusy) {
				mImageLoader.categoryloadImage(context.localhost() + categorylist.get(position).get("_img_url").toString(), this, viewHolder);
				...
				try {
					...
					viewHolder.textview2.setText("Tel：" + spStr[0]);
				} catch (Exception e) {
				...
				Bitmap bitmap = mImageLoader.getBitmapFromCache(context.localhost()+ categorylist.get(position).get("_img_url").toString());
				if (bitmap != null) {
